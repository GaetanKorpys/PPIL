...
#include "Constante.h"
#include <fstream>
ChargementCOR::ChargementCOR(ChargementCOR* s) :_suivant(s) {}

string ChargementCOR::getInfoForme(const string& idForme)const {
	ifstream fichier(SAUVGARDE, ios::in);
	string ligne;
...
Vecteur2D ChargementCOR::stringToPoint(const string& infoPoint)const {
...
Forme* ChargementCOR::resoudre(const string& idForme, const string& info)const {
	string infoForme;
	...
	return _suivant->resoudre(idForme, infoForme);
}